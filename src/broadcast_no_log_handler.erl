-module(broadcast_no_log_handler).
...
processBody(PostBody, Req0) ->
    ...
    #{<<"serverId">> := ServerId,<<"clientId">> := ClientId, <<"region">> := Region, <<"actionCode">> := ActionId, <<"messageBody">> := Body} ->
        Message = lists:concat([binary_to_list(ClientId),"|",binary_to_list(Region),"|",integer_to_list(ActionId),"|",binary_to_list(Body)]),
        broadcast_without_log(list_to_atom(binary_to_list(ServerId)),Message,Req0);
    _ ->
        cowboy_req:reply(400, #{<<"content-type">> => <<"application/json; charset=utf-8">>}, <<"Invalid or missing parameter">>, Req0)
    end.
    
broadcast_without_log(ServerName, Message,Req0) ->
    case global:whereis_name(ServerName) of
        ...
        _->
            global:send(ServerName,{broadcast_no_log,Message, self()}),
            receive
            {success, Published_Message} ->
                StringWithEscapedBackslashes = lists:map(fun(Char) ->
                    case Char of
                        $\" -> "\\\"";
                        _ -> [Char]
                    end
                end, Published_Message),
                Reply = list_to_binary(lists:concat(["{\"Broadcasted\": \"",StringWithEscapedBackslashes,"\"}"])),
